...
module.exports = {
  ...
  async execute(interaction) {
    ...
    for (const r of guildReminders.slice(0, 14)) {
      const time = DateTime.fromISO(r.datetime || '', { zone: r.timezone || 'utc' });
      const when = time.isValid ? time.toLocaleString(DateTime.DATETIME_MED_WITH_WEEKDAY) + ` (${r.timezone})` : 'Unknown time';
